...
+ (instancetype)initWithFrame:(CGRect)frame imageArr:(NSArray *)imageArr numberRow:(int)numberRow widthClearance:(int)widthMargin{
    ...
    //    NSLog(@"number = %d  imageCount = %d",imageShow.number,imageShow.imageCount);
    return imageShow;
}
...
- (void)frame:(CGRect)frame imageWithArr:(NSArray *)imageArr{
    ...
    UICollectionView *imageColl = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, screenWidth, CGRectGetHeight(self.bounds)) collectionViewLayout:layout];
    imageColl.delegate = self;
    imageColl.dataSource = self;
    imageColl.backgroundColor = [UIColor redColor];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    return CGSizeMake((screenWidth - 2 * _widthMargin - (ImageWidthClearance * _number - 1)) / _number, (screenWidth - 2 * _widthMargin - (ImageWidthClearance * _number - 1)) / _number);
}
